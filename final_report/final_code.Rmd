...
newname_list = name_list[1:9]
eval(parse(text = paste("ggplot(data=newplot_year_norm, aes(date))", paste((sapply(c(1:10)[colnames(plot_year_norm) %in% newname_list], ggyear_norm)), collapse = "")))) + labs(title="Year count probability for count > 1000", x ="Time", y = "Probability in year", colour = "Statistical methods")
# remove clustering and independent component
newnamelist = name_list[1:7]
eval(parse(text = paste("ggplot(data=newplot_year_norm, aes(date))", paste((sapply(c(1:10)[colnames(plot_year_norm) %in% newnamelist], ggyear_norm)), collapse = "")))) + labs(title="Year count probability for count > 1000", x ="Time", y = "Probability in year", colour = "Statistical methods")
```
